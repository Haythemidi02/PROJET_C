...
void ajouter_lavage(char matricule[10],int choix_lavage,FILE*L_fichier) {
    ...
    fprintf(L_fichier, "%d %s %f %d %s %d %ld %ld\n",
            nouveau_lavage.id_lavage,nouveau_lavage.type,nouveau_lavage.prix,nouveau_lavage.immatriculation,
            nouveau_lavage.statut,nouveau_lavage.etat_recup, nouveau_lavage.datedeb, nouveau_lavage.datefin);
...
void afficher_lavages(FILE* L_fichier) {
    ...
    while (fscanf(L_fichier, "%d %s %f %s %d %d %ld %ld",
                  ...
                  &nouveau_lavage.datefin) == 8) {
        struct tm *tm_datedeb = localtime(&nouveau_lavage.datedeb);
        struct tm *tm_datefin = localtime(&nouveau_lavage.datefin);
        char date_deb_str[20], date_fin_str[20];
        strftime(date_deb_str, sizeof(date_deb_str), "%Y-%m-%d %H:%M:%S", tm_datedeb);
        strftime(date_fin_str, sizeof(date_fin_str), "%Y-%m-%d %H:%M:%S", tm_datefin);
        printf("| %-10d | %-20s | %-6.2f | %-18s | %-6d | %-10d | %-19s | %-19s |\n",
               nouveau_lavage.id_lavage,
               nouveau_lavage.type,
               nouveau_lavage.prix,
               nouveau_lavage.immatriculation,
